...
public class Woolie extends Thread {
    ...
    /** The time a Woolie must wait when crossing the course for each unit
     *  of time */
    ...
    /** The amount of time it takes a woolie to cross the course if no falls occur*/
    ...
    public Woolie(int id, ObstacleCourse obstacleCourse, Kraken kraken) {
        ...
        this.kraken = kraken;
        this.crossingTime = WoolieWipeout.nextInt(MIN_CROSSING_TIME, MAX_CROSSING_TIME);
        this.crossingTimeRemaining = crossingTime;
        this.totalCrossingTime = 0;
        this.timesFallen = 0;
    }
    ...
    public void run()  {
        ...
        int loopCounter = 0;
        while (loopCounter < this.crossingTime){
            if (this.obstacleCourse.isRunning()){                       //Check that obstacle course is running
                if (this.preserver.isPresent()) {
                    Preserver p = this.preserver.get();
                    p.use();
                }
                int fallOffChance = WoolieWipeout.nextInt(1,100);
                if (fallOffChance <= CHANCE_TO_FALL) {                  //5% chance to fall off
                    System.out.println("\tWOOLIE:" + this + " fell off the course");
                    this.obstacleCourse.fallOff();
                    this.timesFallen++;
                } else {                                                //If woolie didn't fall, cross for one unit of time
                    try {
                        System.out.println("\tWOOLIE:" + this + " crossing course");
                        this.crossingTimeRemaining -= 1;
                        this.totalCrossingTime++;
                        loopCounter++;
                ...
                //Leave course
                this.crossingTimeRemaining = this.crossingTime;
